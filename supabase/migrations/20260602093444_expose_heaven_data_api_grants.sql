...
-- conflict target for public and private upserts. Clean duplicates, then use NULLS NOT DISTINCT
-- identity indexes so PostgREST on_conflict works consistently when user_id/profile_id is null.
with ranked as (
  select id,
         row_number() over (
           partition by user_id, symbol, tf, profile_id, params
...
create unique index if not exists strategy_evaluations_identity_unique
  on public.strategy_evaluations(user_id, symbol, tf, profile_id, params) nulls not distinct;
...
alter view public.v_palmares_best set (security_invoker = true);
