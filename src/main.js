...
function renderStrategyDetailIntoModal(res, ctx){ try{
  ...
  let resCmp=null, eqCmp=null; try{ const pHeaven={ ...(window.lbcOpts||{}) }; resCmp = runBacktestSliceFor(bars, sIdx, eIdx, conf, pHeaven, true); eqCmp = (resCmp.eqSeries||[]).map(x=>({ time:x.time, equity:x.equity })); }catch(_){ resCmp=null; eqCmp=null; }
  try{ if(eqCmp&&eqCmp.length){ drawEquityCompare(canEquity, eq, eqCmp); registerChart('detailEquity', { type:'equity', eq1:eq, eq2:eqCmp }); } else { drawEquity(canEquity, eq); registerChart('detailEquity', { type:'equity', eq1:eq }); } }catch(_){ drawEquity(canEquity, eq); registerChart('detailEquity', { type:'equity', eq1:eq }); }
  drawDD(canDD, eq);
...
if(detailCtxEl){ const gtxt = (ctx && ctx.gen && ctx.gen>1)? ` • Gen ${ctx.gen}` : ''; detailCtxEl.textContent = `${symbolToDisplay(sym)} • ${tf} — ${name}${gtxt} — PF ${(res.profitFactor===Infinity?'∞':(+res.profitFactor||0).toFixed(2))} • Trades ${res.tradesCount}`; }
  ...
  try{
    ...
    ['detailEquity','detailHist','detailRollPF','detailRollWin','detailRollRR','detailRollExp','detailWeekly','detailDOWHour','detailDOWHourLong','detailDOWHourShort'].forEach(addExportBtns);
