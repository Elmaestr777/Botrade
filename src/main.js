...
function drawDurations(canvas, arrMin){ if(!canvas||!Array.isArray(arrMin)||!arrMin.length) return; const ctx=canvas.getContext('2d'); const w=canvas.width, h=canvas.height; ctx.clearRect(0,0,w,h); __drawText(ctx, w/2, 12, 'Durée des trades (minutes)', 'center'); const padL=36, padR=10, padT=18, padB=22; const sorted=arrMin.slice().sort((a,b)=>a-b); const p95=sorted[Math.floor(sorted.length*0.95)]||sorted[sorted.length-1]; const maxVal=Math.max(1, p95); const bins=20; const hist=new Array(bins).fill(0); for(const v of arrMin){ const c=Math.min(v, maxVal); let b=Math.floor(c/maxVal*(bins)); if(b>=bins) b=bins-1; hist[b]++; }
  ctx.strokeStyle=__clr().border; ctx.beginPath(); ctx.moveTo(padL, padT); ctx.lineTo(padL, h-padB); ctx.lineTo(w-padR, h-padB); ctx.stroke(); const mcount=Math.max(...hist,1); for(let i=0;i<bins;i++){ const xx=i/bins*(w-padL-padR)+padL; const hh=(hist[i]/mcount)*(h-padT-padB); ctx.fillStyle='#2563eb'; ctx.fillRect(xx, h-padB-hh, (w-padL-padR)/bins-2, hh); } __drawText(ctx, padL, h-6, '0', 'left'); __drawText(ctx, w-8, h-6, `${maxVal.toFixed(0)}+`, 'right'); }

function drawStreaks(canvas, winCounts, loseCounts){ if(!canvas) return; const ctx=canvas.getContext('2d'); const w=canvas.width, h=canvas.height; ctx.clearRect(0,0,w,h); __drawText(ctx, w/2, 12, 'Séquences victoires/défaites', 'center'); const padL=30, padR=10, padT=18, padB=22; const maxLen=Math.max(winCounts.length, loseCounts.length); const maxVal=Math.max(1, ...winCounts, ...loseCounts); ctx.strokeStyle=__clr().border; ctx.beginPath(); ctx.moveTo(padL, padT); ctx.lineTo(padL, h-padB); ctx.lineTo(w-padR, h-padB); ctx.stroke(); const barW=(w-padL-padR)/Math.max(1,maxLen); for(let i=0;i<maxLen;i++){ const x0=padL+i*barW; const wHalf=barW/2-2; const wv=winCounts[i]||0, lv=loseCounts[i]||0; const hhW = (wv/maxVal)*(h-padT-padB); const hhL = (lv/maxVal)*(h-padT-padB); ctx.fillStyle='#10b981'; ctx.fillRect(x0+2, h-padB-hhW, wHalf, hhW); ctx.fillStyle='#ef4444'; ctx.fillRect(x0+2+wHalf, h-padB-hhL, wHalf, hhL); __drawText(ctx, x0+barW/2, h-6, String(i), 'center'); } __drawText(ctx, w-8, padT+2, 'Vert: Win  Rouge: Loss', 'right'); }
...
const groups=(function(){ const t=(res.trades||[]).slice().sort((a,b)=> (a.exitTime||0)-(b.exitTime||0)); const map=new Map(); function keyOf(e){ return `${e.dir}|${e.entryTime}|${e.entry}|${e.initSL}`; } for(const ev of t){ const k=keyOf(ev); let g=map.get(k); if(!g){ g={ entryTime:ev.entryTime, exitTime:ev.exitTime, net:0, dur:0, dir:ev.dir||'long', eq0: (Number(ev.eqBefore)||null) }; map.set(k,g); } g.net += Number(ev.net)||0; if(Number.isFinite(ev.exitTime)&&Number.isFinite(ev.entryTime)) g.dur = Math.max(g.dur, ev.exitTime-ev.entryTime); if(Number.isFinite(ev.exitTime)) g.exitTime = ev.exitTime; if(g.eq0==null && Number.isFinite(ev.eqBefore)) g.eq0 = Number(ev.eqBefore); }
...
const positions = groups.length||0; const expNet = positions? groups.reduce((s,g)=> s+(Number(g.net)||0),0)/positions : 0; const avgDurMin = positions? (groups.reduce((s,g)=> s+(Number(g.dur)||0),0)/positions/60) : 0; const bestNet = positions? Math.max(...groups.map(g=> Number(g.net)||0)) : 0; const worstNet = positions? Math.min(...groups.map(g=> Number(g.net)||0)) : 0;
  ...
  const rrByPos=(function(){ const map=new Map(); const t=(res.trades||[]); function keyOf(e){ return `${e.dir}|${e.entryTime}|${e.entry}|${e.initSL}`; } for(const ev of t){ const k=keyOf(ev); let arr=map.get(k); if(!arr){ arr=[]; map.set(k,arr); } if(Number.isFinite(ev.rr)) arr.push(Number(ev.rr)); }
    const out=[]; for(const g of groups){ const k=`${g.dir}|${g.entryTime}|${(res.trades||[])[0]?.entry}|${(res.trades||[])[0]?.initSL}`; const arr=map.get(k)||[]; const m=arr.length? (arr.reduce((x,y)=>x+y,0)/arr.length) : null; out.push(m); } return out; })();
...
drawRobust(canRob, compN, edgeN);
  // Rolling charts
  try{ drawLineChart(canRollPF, rollPF, { title:'Rolling PF (fenêtre 30)', yMin:0, yMax: Math.max(3, Math.min(5, Math.max(...rollPF,3))), fmt:(v)=> (v===Infinity?'∞':v.toFixed(2)) }); }catch(_){ }
  try{ drawLineChart(canRollWin, rollWin, { title:'Rolling Win% (fenêtre 30)', yMin:0, yMax:100, fmt:(v)=> v.toFixed(0)+'%' }); }catch(_){ }
  try{ drawLineChart(canRollRR, rollRR, { title:'Rolling Avg R:R (fenêtre 30)', yMin:0, yMax: Math.max(2, Math.max(...rollRR,1.5)), fmt:(v)=> v.toFixed(2) }); }catch(_){ }
  try{ drawLineChart(canRollExp, rollExp, { title:'Rolling Expectancy (USD, fen.30)', yMin: Math.min(0, Math.min(...rollExp,0)), yMax: Math.max(0, Math.max(...rollExp,0)), fmt:(v)=> v.toFixed(2) }); }catch(_){ }
  // Diagnostics charts
