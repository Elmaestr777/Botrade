...
function mergeDuplicateTargets(list){ const out=[]; const seen=new Map(); for(const raw of Array.isArray(list)?list:[]){ const price=Number(raw&&raw.price); if(!Number.isFinite(price)) {continue;} const key=price.toFixed(8); const qty=(raw.qty!=null && isFinite(raw.qty))? Number(raw.qty):0; const existing=seen.get(key); if(existing){ existing.qty += qty; } else { const t={...raw, price, qty}; seen.set(key,t); out.push(t); } } return out; }
function generateHeavenSignalsOnBars(opts, bars){
  ...
  const ratios=[]; if(opts.ent382) ratios.push(0.382); if(opts.ent500) ratios.push(0.5); if(opts.ent618) ratios.push(0.618); if(opts.ent786) ratios.push(0.786);
  const flips=new Set(lb.flips); let pivotIdx=-1, pendingFib=null;
  for(let i=1;i<bars.length;i++){
    while(pivotIdx+1<piv.length && piv[pivotIdx+1].idx+opts.prd<=i) pivotIdx++;
    const seg=(pivotIdx>=1)? { a:piv[pivotIdx-1], b:piv[pivotIdx], dir:piv[pivotIdx].price>piv[pivotIdx-1].price?'up':'down' } : null;
    if(flips.has(i)){
      const dir=lb.trend[i]===1?'long':'short';
      if(useLB && i+1<bars.length) signals.push({ idx:i+1, dir, type:'LB' });
      if(useFib && seg){ const move=Math.abs(seg.b.price-seg.a.price); pendingFib={ dir, levels:ratios.map(r=>seg.dir==='up'?seg.b.price-move*r:seg.b.price+move*r) }; }
      else if(useFib) pendingFib=null;
    }
    if(useFib && pendingFib && i+1<bars.length){
      for(const lvl of pendingFib.levels){ const hit=pendingFib.dir==='long'? (bars[i].low<=lvl && (opts.confirmMode!=='Bounce' || bars[i].close>lvl)) : (bars[i].high>=lvl && (opts.confirmMode!=='Bounce' || bars[i].close<lvl)); if(hit){ signals.push({ idx:i+1, dir:pendingFib.dir, type:'Fib' }); pendingFib=null; break; } }
    }
  }
  signals.sort((a,b)=> a.idx===b.idx ? (a.type==='LB' ? -1 : 1) : a.idx-b.idx);
  return signals;
}
function lastTwoPivotsBefore(piv, idx, prd){ let b=null,a=null; for(let i=piv.length-1;i>=0;i--){ if(piv[i].idx + prd < idx){ if(!b) {b=piv[i];} else { a=piv[i]; break; } } } return (a&&b)? {a,b}: null; }
...
function runBacktestWithBars(opts, fromIdx, toIdx, forcedSignals=null, forcedPivots=null){ if(fromIdx<0||toIdx<0||fromIdx>=toIdx) {return null;} const piv = Array.isArray(forcedPivots)? forcedPivots : (pivotCache.get(opts.prd) || computePivots(BARS, opts.prd)); if(!Array.isArray(forcedPivots)) {pivotCache.set(opts.prd, piv);} const baseSignals = Array.isArray(forcedSignals)? forcedSignals : generateHeavenSignalsOnBars(opts, BARS); const seenEntryIdx=new Set(); const signals=baseSignals.filter(s=>{ if(s.idx<fromIdx || s.idx>toIdx || seenEntryIdx.has(s.idx)) return false; seenEntryIdx.add(s.idx); return true; });
  ...
  for(let i=0;i<signals.length;i++){
    const sig=signals[i]; const endBound = i+1 < signals.length ? Math.min(toIdx, signals[i+1].idx-1) : toIdx; const eqBefore=equity; const res=simulateTradeFromSignalOnBars(sig, endBound, piv, opts, equity, SETTINGS, BARS); if(res){ trades.push(res); equity += res.pnl; returns.push(eqBefore>1e-9? (res.pnl/eqBefore):0); eqSeries.push(equity); if(res.pnl>=0) {grossProf+=res.pnl;} else {grossLoss+=res.pnl;} if(res.pnl>0) {wins++;} if(Number.isFinite(res.rr)){ sumRR+=res.rr; cntRR++; } if(equity>peak) {peak=equity;} const dd=(peak-equity)/Math.max(1e-9,peak); if(dd>maxDD) {maxDD=dd;} const dda=(peak-equity); if(dda>maxDDAbs) {maxDDAbs=dda;} try{ holdSumSec += Math.max(0, (BARS[res.exitIdx].time - BARS[res.entryIdx].time)); }catch(e){} }
  }
