...
function runBacktestSliceFor(bars, sIdx, eIdx, conf, params){
  ...
  function computeSLFromLadder(dir, entry, i){
    try{
      ...
      for(const t of params.sl){ const typ=(t&&t.type)||'Percent'; let price=null;
        ...
        else if(typ==='EMA'){ const len=Math.max(1, parseInt(((t&&t.emaLen)!=null? t.emaLen : (params.emaLen||55)),10)); let ema=slEmaCache.get(len); if(!ema){ ema=emaCalc(bars, len); slEmaCache.set(len, ema); } const v=ema[Math.min(i, ema.length-1)]; if(isFinite(v)) price=v; }
  ...
  for(let i=Math.max(1,sIdx); i<=eIdx; i++){
    ...
    try{
      ...
      if(pos && pos.targets && pos.tpIdx < pos.targets.length){
        while(pos && pos.tpIdx < pos.targets.length){
          ...
          pos.tpIdx++;
          if(!tpCompound || pos.qty<=1e-12){ pos=null; break; }
        }
      }
    ...
    }catch(_){ /* swallow and continue next bar */ }
  }
