...
async function onBar(st, bar){
  ...
  if(st.pos && !Array.isArray(st.pos.targets)){ const initSL=Number(st.pos.initSL!=null?st.pos.initSL:st.pos.sl); const riskAbs=Math.abs(Number(st.pos.entry)-initSL); st.pos.targets=buildTargets(p, seg, st.pos.dir, Number(st.pos.entry), riskAbs, arr, i); }
  if(!st.pos){ if(trendNow!==trendPrev){
    if(p.entryMode!=='Fib Retracement'){
      ...
      if(qty>1e-12){ const targets=buildTargets(p, seg, dir, entry, riskAbs, arr, i); st.pos={ dir, entry, sl, initSL:sl, qty, initQty:qty, entryTime:bar.time, beActive:false, tpIdx:0, targets, hiSince:bar.high, loSince:bar.low };
